...
static inline void	bigint_mul_aux(t_bigint *res, const t_bigint *l, uint64_t s_i, uint32_t i)
{
	uint64_t		over;
	uint64_t		mul;
	uint32_t		j;
...
void				bigint_mul(t_bigint *res, const t_bigint *b1, const t_bigint *b2)
{
	const t_bigint	*l;
	const t_bigint	*s;
	uint32_t		res_max;
	...
	res->length = res_max > 0 && res->block[res_max - 1] == 0 ? res_max - 1 : res_max;
}

void				bigint_mul_int(t_bigint *res, const t_bigint *b1, uint32_t b2)
{
	uint32_t		over;
	uint32_t		i;
	uint64_t		mul;
