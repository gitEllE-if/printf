											...
											const int32_t exp_in, int32_t *exp_10)
{
	...
	tmp1 = &(*bigints)[3];
	tmp2 = &(*bigints)[4];
	*exp_10 = (int32_t)(ft_ceil((double)((int32_t)mantis_bit + exp_in) * LOG10_2 - 0.69));
	if (*exp_10 > 0)
...
static uint32_t	dragon4_get_digit(char **buf, int32_t exp_10, t_bigint **scale, t_bigint **scaled_val)
{
	int32_t		hi_block;
	uint32_t	shift;
	int32_t		exp_cut;
...
static int32_t	dragon4_round(char **buf_cur, char *buf, uint32_t digit, t_bigint **bigints)
{
	int32_t		cmp;
	int32_t		round_down;
	int32_t		exp_inc;
...
uint32_t		dragon4_solve(t_bigint *bigints, t_float80 *num, char *buf, int32_t *exp_out)
{
	t_bigint	*scale;
	t_bigint	*scaled_val;
	char		*buf_cur;
